<section id="one" class="wrapper withimg">
    <div class="box-1">
        <header class="major">
            <h2>活動日程</h2>
            <p>Timeline</p>
        </header>
        <div class="timelinewrapper">
            <ol class="ProgressBar">
                <li class="ProgressBar-step">
                    <svg class="ProgressBar-icon"></svg>
                    <span class="ProgressBar-stepLabel">籌備中</span>
                </li>
                <li class="ProgressBar-step">
                    <span class="ProgressBar-stepLabel upperdate">3/21</span>
                    <span class="ProgressBar-stepLabel uppertext">報名開始</span>
                    <svg class="ProgressBar-icon"></svg>
                    
                </li>
                <li class="ProgressBar-step">
                    <svg class="ProgressBar-icon"></svg>
                    <span class="ProgressBar-stepLabel">報名截止</span>
                    <span class="ProgressBar-stepLabel lowerdate" >5/19</span>
                </li>
                <li class="ProgressBar-step">
                    <span class="ProgressBar-stepLabel upperdate" >5/24</span>
                    <span class="ProgressBar-stepLabel uppertext">名單出爐</span>
                    <svg class="ProgressBar-icon"></svg>
                </li>
                <li class="ProgressBar-step">
                    <svg class="ProgressBar-icon"></svg>
                    <span class="ProgressBar-stepLabel">營期開始</span>
                    <span class="ProgressBar-stepLabel lowerdate">7/15</span>
                </li>
                <li class="ProgressBar-step">
                    <span class="ProgressBar-stepLabel upperdate">7/19</span>
                    <span class="ProgressBar-stepLabel uppertext">圓滿結束</span>
                    <svg class="ProgressBar-icon"></svg>
                </li>
            </ol>

            <div class="timer">
                <p id='textLayout'>距離 報名截止 還有 <span id='timer_js'></span></p>
            </div>
        </div>
    </div>
</section>